...
class GanModel(BaseModel):
    ...
    def backward_D_P(self):
        ...
        for i in range(self.opt.patchD_3):
            lossD_P += self.backward_D_basic(self.netD_P, self.real_patch1[i], self.fake_patch1[i])
    ...
    def backward_G(self, epoch):
        ...
        if self.opt.patchD_3 > 0:
            for i, data in self.fake_patch1:
    ...
    def forward(self):
        ...
        self.fake_patch = self.fakeB[:, :, h_offset:h_offset + self.opt.patchsize,
                          w_offset:w_offset + self.opt.patchsize]
        self.real_patch = self.realB[:, :, h_offset:h_offset + self.opt.patchsize,
                          w_offset:w_offset + self.opt.patchsize]
        self.input_patch = self.realA[:, :, h_offset:h_offset + self.opt.patchsize,
                           w_offset:w_offset + self.opt.patchsize]
